...
#include "../common/fdt_board_info.h"
...
int board_late_init(void)
{
	...
	/* We don't need to create our own custom struct boot_mode structure
	 * since calling this will add a custom struct and then the default
	 * bmodes defined in mx6/soc.c.
	 * The two modes we need on this platform are ecspi1:1 and usb.
	 * If a customer wanted to boot from SD or eMMC for some special
	 * recovery mode, that is still possible with the default esdhc*
	 * modes.
	 */
	...
	rev[0] = ts4900_pcb_rev_char(straps);
	env_set("pcb_revision", rev);
...
{
	const char env_conf[] = "ts4900-lts-v1";

	if (ets_fdt_set_bom_options(blob, (u32)env_get_hex("bom_options", -1)) < 0)
		goto ft_out;

	if (ets_fdt_set_bbid(blob, (u32)env_get_hex("baseboard_id", -1)) < 0)
		goto ft_out;

	if (ets_fdt_set_bbrev(blob, (u32)env_get_hex("baseboard_rev", -1)) < 0)
		goto ft_out;

	if (ets_fdt_set_pcb_revision(blob, env_get("pcb_revision")) < 0)
		goto ft_out;

	if (fdt_find_and_setprop(blob, "/chosen", "u-boot,env-config",
				 env_conf, strlen(env_conf) + 1, 1) < 0)
		goto ft_out;

	return 0;

ft_out:
	printf("Could not set all FDT properties!\n");
	return -1;
}
