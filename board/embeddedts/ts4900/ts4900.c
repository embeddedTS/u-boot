...
iomux_v3_cfg_t const wifi_pads[] = {
	IOMUX_PADS(PAD_ENET_RXD0__GPIO1_IO27 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* BT_EN */
	IOMUX_PADS(PAD_ENET_RXD1__GPIO1_IO26 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* WIFI_EN */
	IOMUX_PADS(PAD_SD1_CMD__GPIO1_IO18 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_CMD */
	IOMUX_PADS(PAD_SD1_CLK__GPIO1_IO20 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_CLK */
	IOMUX_PADS(PAD_SD1_DAT0__GPIO1_IO16 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_D0 */
	IOMUX_PADS(PAD_SD1_DAT1__GPIO1_IO17 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_D1*/
	IOMUX_PADS(PAD_SD1_DAT2__GPIO1_IO19 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_D2 */
	IOMUX_PADS(PAD_SD1_DAT3__GPIO1_IO21 | MUX_PAD_CTRL(SPI_PAD_CTRL)), /* SD1_D3 */
};

iomux_v3_cfg_t const misc_pads[] = {
	IOMUX_PADS(PAD_EIM_A17__GPIO2_IO21 | MUX_PAD_CTRL(NO_PAD_CTRL)), // off_bd_reset
	IOMUX_PADS(PAD_GPIO_2__GPIO1_IO02 | MUX_PAD_CTRL(NO_PAD_CTRL)), // Red LED
	IOMUX_PADS(PAD_EIM_CS1__GPIO2_IO24 | MUX_PAD_CTRL(NO_PAD_CTRL)), // Green LED
	IOMUX_PADS(PAD_EIM_RW__GPIO2_IO26 | MUX_PAD_CTRL(NO_PAD_CTRL)), // MODE2
	IOMUX_PADS(PAD_EIM_OE__GPIO2_IO25 | MUX_PAD_CTRL(NO_PAD_CTRL)), // BD_ID_DATA
	IOMUX_PADS(PAD_SD4_DAT3__GPIO2_IO11 | MUX_PAD_CTRL(NO_PAD_CTRL)), // Rev A/C strap
	IOMUX_PADS(PAD_CSI0_DAT19__GPIO6_IO05 | MUX_PAD_CTRL(NO_PAD_CTRL)), // Rev D strap
	IOMUX_PADS(PAD_ENET_TXD1__GPIO1_IO29 | MUX_PAD_CTRL(NO_PAD_CTRL)), // Rev E strap
	IOMUX_PADS(PAD_EIM_D23__GPIO3_IO23 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_RTC
	IOMUX_PADS(PAD_EIM_A16__GPIO2_IO22 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_USB_5V
	IOMUX_PADS(PAD_GPIO_1__USB_OTG_ID | MUX_PAD_CTRL(OTG_ID_PAD_CTRL)), // USB_OTG_ID
};
...
/* XXX: TODO: Match this with sabreauto */
...
iomux_v3_cfg_t const fpga_pads[] = {
	IOMUX_PADS(PAD_CSI0_DATA_EN__GPIO5_IO20 | MUX_PAD_CTRL(SPI_PAD_CTRL)), // FPGA_DONE
	IOMUX_PADS(PAD_CSI0_VSYNC__GPIO5_IO21 | MUX_PAD_CTRL(SPI_PAD_CTRL)),// FPGA_RESET
	IOMUX_PADS(PAD_CSI0_DAT16__GPIO6_IO02 | MUX_PAD_CTRL(SPI_PAD_CTRL)), // FPGA_SPI_CS#
	...
	IOMUX_PADS(PAD_CSI0_DAT11__GPIO5_IO29 | MUX_PAD_CTRL(SPI_PAD_CTRL)), // offboard CS#
	IOMUX_PADS(PAD_GPIO_3__XTALOSC_REF_CLK_24M | MUX_PAD_CTRL(NO_PAD_CTRL)), // FPGA CLK
};
...
iomux_v3_cfg_t const usdhc2_pads[] = {
	...
	IOMUX_PADS(PAD_EIM_EB0__GPIO2_IO28 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_SD_POWER
};
...
int misc_init_r(void)
{
	...
	env_set("model", "4900");
	//env_set("rcause", get_reset_cause(1));
	//rev[0] = board_rev();
...
struct i2c_pads_info i2c_pad_info0 = {
	.scl = {
		.i2c_mode  = IOMUX_PADS(PAD_EIM_D21__I2C1_SCL | MUX_PAD_CTRL(I2C_PAD_CTRL)),
		.gpio_mode = IOMUX_PADS(PAD_EIM_D21__GPIO3_IO21 | MUX_PAD_CTRL(I2C_PAD_CTRL)),
		.gp = TS4900_SCL
	},
	.sda = {
		.i2c_mode = IOMUX_PADS(PAD_EIM_D28__I2C1_SDA | MUX_PAD_CTRL(I2C_PAD_CTRL)),
		.gpio_mode = IOMUX_PADS(PAD_EIM_D28__GPIO3_IO28 | MUX_PAD_CTRL(I2C_PAD_CTRL)),
		.gp = TS4900_SDA
	}
};
#endif
