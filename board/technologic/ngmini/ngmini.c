...
#define NGMINI_EN_SDPWR		MX28_PAD_PWM3__GPIO_3_28
#define NGMINI_SDBOOT_JP	MX28_PAD_LCD_D12__GPIO_1_12
#define NGMINI_UBOOT_JP		MX28_PAD_LCD_D11__GPIO_1_11

#define NGMINI_OFFBDSPI_SELN	MX28_PAD_GPMI_RDN__GPIO_0_24

#define NGMINI_EN_USBPWR	MX28_PAD_LCD_CS__GPIO_1_27
...
int misc_init_r(void)
{
	...
	gpio_direction_output(NGMINI_EN_USBPWR, 1);

	setenv("model", "NGmini");

	gpio_direction_input(NGMINI_SDBOOT_JP);
	sdboot = gpio_get_value(NGMINI_SDBOOT_JP);

	if(sdboot) setenv("jpsdboot", "off");
	else setenv("jpsdboot", "on");

	gpio_direction_input(NGMINI_UBOOT_JP);
	sdboot = gpio_get_value(NGMINI_UBOOT_JP);
...
static int ngmini_mmc_cd(int id) {
	return 1;
}
...
{
	int ret;
	mxs_iomux_setup_pad(NGMINI_EN_SDPWR);

	gpio_direction_output(NGMINI_EN_SDPWR, 1); // EN_SD_POWER#
	udelay(1000);
	gpio_direction_output(NGMINI_EN_SDPWR, 0);

	/* SD card */
	ret = mxsmmc_initialize(bis, 0, NULL, ngmini_mmc_cd);
	if(ret != 0) {
		printf("SD controller initialized with %d\n", ret);
	}

	/* eMMC */
	ret = mxsmmc_initialize(bis, 1, NULL, ngmini_mmc_cd);
	if(ret != 0) {
...
static int set_mx28_spi(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	switch(mode) {
	  case 0:
		gpio_direction_input(NGMINI_OFFBDSPI_SELN);
		break;
	  case 1:
		gpio_direction_output(NGMINI_OFFBDSPI_SELN, 1);
		break;
	  case 2:
		gpio_direction_output(NGMINI_OFFBDSPI_SELN, 0);
		break;
	  case 3:
		gpio_direction_input(NGMINI_OFFBDSPI_SELN);
		break;
